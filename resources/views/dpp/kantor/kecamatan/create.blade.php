...
<div class="row">
    <div class="col-md-6">
        <h4 class="page-title">Tambah Kantor PAC Kecamatan {{ $daerah->name }}</h4>
    </div>
    
    

    <div class="col-sm-6 text-right m-b-20">
         <input type="hidden" id="kab_code" value="{{ $kabupaten_code }}"/>
        <input type="hidden" id="camat_code" value="{{ $kecamatan_code }}"/>
    </div>
</div>
    ...
    <div class="col-sm-12">
        <div class="card">
            ...
            <div class="card-body pl-4">
                <form method="post" action="{{ route('dpp.kantor.store', $daerah->id) }}" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="id_daerah" value="{{ $daerah->id_kec }}">

                    <div class="row mb-4">
                        <label for="alamat" class="col-sm-2 col-form-label" placeholder="Alamat">Alamat</label>
                        <div class="col-sm-10">
                            <textarea type="text" class="form-control {{ $errors->has('alamat') ? 'border border-danger' : '' }}" id="alamat" name="alamat" rows="5">{{ old('alamat') }}</textarea>
                            ...
                            <select name="provinsi" id="provinsi" class="form-control select2" style="width: 100%; height:36px;">
                                ...
                                @foreach($provinsi as $data => $row)
                                 @if($data == $provinsi_code)
                                    <option  id="selected_provinsi" value="{{ $data }}" name="{{ $row }}">{{ $row }}</option>
                                    @else
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="wa_kantor" maxlength="15" value="{{ old('wa_kantor') }}" class="form-control" id="wa_kantor" placeholder="Nomor WhatsApp Kantor">
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="fax" maxlength="10" value="{{ old('fax') }}" class="form-control" id="fax" placeholder="Nomor Fax">
                            @if($errors->has('fax'))
                                <strong class="text-danger">
                                    {{ $errors->first('fax') }}
                                </strong>
                    ...
                    <div id="hidden_div" style="display:none;">
                        ...
                        <div class="row mb-4">
                            <label for="cap_kantor" class="col-sm-2 col-form-label">Tanggal Sewa Selesai</label>
                            <div class="col-sm-10">
                    ...
                    <div id="hidden_div1" style="display:none;">
                        <div class="row mb-4">
                            <label for="cap_kantor" class="col-sm-2 col-form-label">Tanggal Pinjam Pakai</label>
                            <div class="col-sm-10">
                                <input type="date" id="tgl_awal"  name="tgl_awal" class="form-control">
                        ...
                        <div class="row mb-4">
                            <label for="cap_kantor" class="col-sm-2 col-form-label">Tanggal Pinjam Pakai Selesai</label>
                            <div class="col-sm-10">
                                <input type="date" id="tgl_selesai"  name="tgl_selesai" class="form-control">
                    ...
                    <div class="row mb-4">
                            ...
                            <div class="col-sm-10">
                                <input type="date" id="tanggal_pengesahan_sk"   name="tanggal_pengesahan_sk" class="form-control"
                                     >
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-sm-10">
                            <input type="hidden" name="is_active" value="1" class="form-control" id="is_active" placeholder="Status Kantor">
...
<script>
    ...
    $("#kecamatan_domisili").change(function () {
     ...
     $(window).load(function(){
        let val = $('select>option#selected_provinsi').prop('selected', true).val();
        $.ajax({
            url: '/dpp/ajax/kab',
            data: {
                val: val
            ...
            type: 'GET',
            success: function (response) {
                var kabupaten_code = $("#kab_code").val();
                var len = response.length;
                ...
                if (len > 1) {
                    for (var i = 0; i < len; i++) {
                        var code = response[i]['id_kab'];
                        var name = response[i]['name'];
                        if(code == kabupaten_code){
                            $("#kab_kota").append("<option id='selected_kabupaten' selected value='" + code + "' name='" + name + "'>" + name + "</option>");
                        }else{
                            $("#kab_kota").append("<option value='" + code + "' name='" + name + "'>" + name + "</option>");
                        }
            ...
            success: function (response) {
                ...
                if (len > 1) {
                    for (var i = 0; i < len; i++) {
                        ...
                        var name = response[i]['name'];
                        if(code == camat_code){
                            $("#kec").append("<option selected id='selected_kecamatan' value='" + code + "' name='" + name + "'>" +name + "</option>");
                        }else{
                            $("#kec").append("<option value='" + code + "' name='" + name + "'>" +name + "</option>");
                        }
                    }
                }
            }
