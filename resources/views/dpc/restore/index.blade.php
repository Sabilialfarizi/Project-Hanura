            ...
            <div style="margin-bottom: 10px;" class="row">
                <div class="col-lg-12 col-md-12 col-xl-12">
                    <button type="submit" onclick="akftifstatus()"
                        style=" margin-left:20px;background-color:#D6A62C; color:#FFFF;" id="button_aktif"
...
<script>
    $(document).ready(function () {
        ...
        $('#pembatalan tbody').on('click', '.cb-child', function () {
            ...
            var data = $('#pembatalan').DataTable().row(tr).data();
            var user = {!!$nik!!};
            user.map(item => {
                if (data.nik == item.nik) {

                    console.log('tes');
                    var content = document.createElement('div');
                    $(".cb-child").prop('checked', false);
                  
                    content.innerHTML = 'Tidak dapat restore Anggota <strong>' + data.name +
                        '</strong> (data nama dan nik ini sudah ada di internal anggota)';
                    swal({
                        content: content,
                        title: 'Gagal',
                        icon: 'error',
...
<script>
    ...
    function akftifstatus() {
        ...
        if (checkbox.length > 0) {
            ...
            $.ajax({
                url: "{{route ('dpc.restore.updateaktif')}}",
                method: 'post',
                data: {
                    ids: semua_id
                },
                success: function (res) {
                        ...
                        swal('Berhasil melakukan Restore');
                    
                }
            });
        } else {
            swal('Silahkan centang kotak untuk anggota yang akan direstore');
        }
    ...
    function nonaktif() {
        ...
        $.ajax({
            url: "{{route ('dpc.restore.updatenonaktif')}}",
            method: 'post',
            data: {
                ids: semua_id
            },
