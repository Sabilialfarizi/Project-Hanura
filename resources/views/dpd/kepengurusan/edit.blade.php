...
<div class="card">
    <div class="card-header">
        <strong>Form Kepengurusan Kabupaten / Kota {{$kabupaten->name}}</strong>
    </div>

    <div class="card-body">
        <form action="{{ "/dpd/kabupaten/$kabupaten->id_kab/kepengurusan/$kepengurusan->id_kepengurusan" }}"
            ...
            <div class="row">
                <div class="col-lg-6">
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" value="{{ $kepengurusan->nama ?? '' }}" name="nama" class="form-control"
                                id="nama" required>
                            @if($errors->has('nama'))
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <select name="jabatan" id="jabatan" class="form-control" onchange="showDiv(this)">
                                style="width: 100%; height:36px;">
                                <option value="" selected disabled hidden>-- Pilih --</option>
                                @foreach($jabatan as $data)
                                <option value="{{ $data->kode }}"
                                    {{ $kepengurusan->jabatan == $data->kode ? 'selected' : '' }}>{{ $data->nama }}  ({{$data->kode}})
                                </option>
                                @endforeach
                            </select>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" onkeypress="return isNumberKey(event)" name="kta" class="form-control"
                                id="kta" value="{{ $kepengurusan->kta ?? '' }}" required>
                            @if($errors->has('kta'))
                            <strong class="text-danger">
                                {{ $errors->first('kta') }}
                            </strong>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" name="no_sk" class="form-control" id="no_sk"
                                value="{{ $kepengurusan->no_sk ?? '' }}" required>
                            @if($errors->has('no_sk'))
                            <strong class="text-danger">
                                {{ $errors->first('no_sk') }}
                            </strong>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <textarea type="number" name="alamat_kantor" class="form-control" id="alamat_kantor"
                                rows="3" required>{{ $kepengurusan->alamat_kantor ?? '' }}</textarea>
